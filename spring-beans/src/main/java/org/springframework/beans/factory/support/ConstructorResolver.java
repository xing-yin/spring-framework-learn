...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		// 1.构造函数参数的确定
		// explicitArgs 通过 getBean 方法传入，比如： getBean(String name,Object...args)
		// 如果 getBean 方法调用时指定方法参数那么直接使用：不仅指定 beanName,还指定构造函数或工厂方法的参数
		if (explicitArgs != null) {
			argsToUse = explicitArgs;
		} else {
			// 如果 getBean 方法调用时没有指定则尝试从配置文件中解析
			...
			for (Constructor<?> candidate : candidates) {
				...
				if (resolvedValues != null) {
					...
					try {
						...
						if (paramNames == null) {
							// 获取参数名称探索器：获取构造函数里的参数名称，如 <constructor-arg name="aa">
				...
				// 探测是否有不确定的构造函数存在，例如不同构造函数的参数父子关系

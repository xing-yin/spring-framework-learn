...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			if (!StringUtils.hasText(beanName)) {
				// 如果不存在 beanName，则根据 spring 提供的命名规则为当前 bean 生成对应的 beanName
				try {
					if (containingBean != null) {
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		if (ele.hasAttribute(SINGLETON_ATTRIBUTE)) {
			...
			// 在嵌入 beanDefitinition 情况下且没有单独指定 scope 属性，则使用父类默认的属性
	 ...
	 * 用法示例
	 * <bean id="myTestBean" class="xxxx.MyTestBean">
	 * <meta key="testStr" value="666"/>
	 * </bean>
	 */
	...
	/**
	 * Decorate(装饰) the given bean definition through a namespace handler, if applicable.
	 *

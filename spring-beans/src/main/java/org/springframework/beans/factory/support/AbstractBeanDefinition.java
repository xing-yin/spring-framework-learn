 ...
 * 分解出 {@link GenericBeanDefinition},{@link RootBeanDefinition}, and {@link ChildBeanDefinition} 的公共属性
 * <p>
 * >[idea]可以从 AbstractBeanDefinition 的一个设计中学习抽象类抽取公共属性，用于给子类继承，从而实现属性/方法的复用
 *
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 表示一个 bean 的实例化依赖另外的 bean 先实例化
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * 设置为 false时，容器在查找自动装配对象时，将不考虑该类，及不会被考虑为 bean 自动装配的候选者，
	 * 但是该 bean 本身可以使用自动装配来注入其他 bean 的
	 */
	...
	/**
	 * 是否是用户自定义的而不是应用本身定义的，创建 AOP 时为 true,程序设置
	 */
	...
	/**
	 * Validate and prepare the given method override. （校验和准备给定的方法覆盖）
	 * Checks for existence of a method with the specified name,
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			// 标记 MethodOverloaded 暂未被覆盖，避免参数类型检查的开销

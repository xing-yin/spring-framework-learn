 ...
 * <p>
 * Spring 的{@link ConfigurableListableBeanFactory} 和{@linkbeandefinitionregistry}接口的默认实现：
 * 一个基于 bean 定义元数据的成熟 bean 工厂，可通过 post-processors 进行扩展。
 *
 ...
 * <p>
 * 典型用法是：在访问bean之前，先注册所有的 bean 定义（可能是从一个 bean 定义文件中读取）。
 * 因此在本地bean定义表中按名称查找 Bean 是一个低成本的操作，操作基于预解析的 bean 定义元数据对象。
 *
 * <p>Note that readers for specific bean definition formats are typically
 * implemented separately rather than as bean factory subclasses: see for example
 * {@link org.springframework.beans.factory.xml.XmlBeanDefinitionReader}.
 * <p>
 * 注意，特定 bean 定义格式的 readers 通常会单独实现而不是作为bean工厂子类：
 * 参见示例{@link org.springframework.beans.factory.xml.XmlBeanDefinitionReader}
 *
 ...
 * <p>
 * 对于{@link org.springframework.beans.factory.ListableBeanFactory}接口有一个可替换的实现，
 * 可以参见{@link StaticListableBeanFactory}，它管理现有的bean实例，而不是基于 bean 定义创建新实例。
 *
 * @author Rod Johnson
...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	/**
	 * Map from dependency type to corresponding(相应的) autowired value.
	 */
